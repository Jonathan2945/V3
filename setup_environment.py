    ...
    def install_system_dependencies(self):
        ...
        try:
            if self.system in system_deps:
                ...
                if success_count > 0 or self.system == 'windows':
                    logger.info("System dependencies installation attempted")
                    return True
                else:
                    logger.warning("System dependency installation failed")
                    return False
            else:
                logger.info("No system dependencies defined for this platform")
                return True
        except Exception as e:
            logger.error(f"System dependencies failed: {e}")
            return False
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"Critical error in setup_environment: {e}")
        sys.exit(1)